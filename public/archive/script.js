...
async function updateStatus(id, newStatus) {
	...
	if (response.ok) {
		const selectElement = document.querySelector(
			`.file-card select[onchange="updateStatus(${id}, this.value)"]`,
		);
...
async function loadFiles(
	...
	const response = await fetch(
		`/files?order=${order}&filterParam=${filterParam}&author=${encodeURIComponent(
			author,
		)}&filename=${encodeURIComponent(filename)}&searchType=${searchType}`,
	);
	if (response.ok) {
		...
		files.forEach((file) =>
			displayFile(file.id, file.filename, file.size, file.status),
		);
...
function displayFile(id, filename, size, status) {
	...
	fileCard.innerHTML = `
        ...
        <div class="file-actions">
            ...
            <select class="status-select ${getStatusClass(
				status,
			)}" onchange="updateStatus(${id}, this.value)">
                <option value="В работе" ${
					status === "В работе" ? "selected" : ""
				}>В работе</option>
                <option value="Отклонён" ${
					status === "Отклонён" ? "selected" : ""
				}>Отклонён</option>
                <option value="Принят" ${
					status === "Принят" ? "selected" : ""
				}>Принят</option>
            </select>
        </div>
    `;
...
async function deleteFile(id, filename) {
	...
	const currentCount = parseInt(
		document.getElementById("fileCount").textContent.split(": ")[1],
	);
	updateFileCount(currentCount - 1);
	if (response.ok) {
...
function openReplaceModal(id, oldFilename) {
	...
	fileInput.onchange = () => {
		...
		document.getElementById(
			"modal-text",
		).textContent = `Заменить ${oldFilename} на ${newFileToReplace.name}?`;
...
// Удаление файла
document
	.getElementById("confirm-replace")
	.addEventListener("click", async function () {
		if (newFileToReplace) {
			...
			if (response.ok) {
				...
				for (let i = 0; i < fileCards.length; i++) {
					if (
						fileCards[i].querySelector("p").textContent ===
						result.oldFilename
					) {
						fileCards[i].querySelector("p").textContent =
							result.filename;
						...
						fileCards[i]
							.querySelector(".replace-button")
							.setAttribute(
								"onclick",
								`openReplaceModal('${result.id}', '${result.filename}')`,
							);
						fileCards[i]
							.querySelector(".delete-button")
							.setAttribute(
								"onclick",
								`deleteFile('${result.id}', '${result.filename}')`,
							);
						fileCards[i]
							.querySelector(".metadata-button")
							.setAttribute(
								"onclick",
...
document
	.getElementById("emptyTrashButton")
	.addEventListener("click", function () {
...
document
	.getElementById("confirmEmptyTrash")
	.addEventListener("click", async function () {
...
document
	.getElementById("cancelEmptyTrash")
	.addEventListener("click", function () {
		document.getElementById("emptyTrashModal").style.display = "none";
	});
...
document
	.getElementById("historyFilterSelect")
